...
const CreateCampaign = () => {
    ...
    const [messageRadio,setmessageRadio] = useState('');
    ...
    const messageTypeValidation = () => {
        if ((messageRadio === 'audio') && (audioMessage === null) && (whichAudio === 'Single')) {
            setp4("active")
            toast.error("Please enter audio file message ")
        }
        else if ((messageRadio === 'audio') && (whichAudio === 'Multiple') && (welcomeFile === null)) {
            setp4("active")
            toast.error("Please enter welcome file message ")

        }
        else if ((messageRadio === 'audio') && (whichAudio === 'Multiple') && (menuFile === null)) {
            setp4("active")
            toast.error("Please enter menu file message ")

        }
        else if ((messageRadio === 'audio') && (whichAudio === 'Multiple') && (thanksFile === null)) {
            setp4("active")
            toast.error("Please enter thanks file message ")

        }
        else if ((messageRadio === 'text') && (ttsTextArea?.length === 0)) {
            setp4("active")
            toast.error("Please enter text field file message ")
        }
        else if ((messageRadio === 'text') && (ttsVoiceTone?.length === 0)) {
            setp4("active")
            toast.error("Please enter voice tone file message ")
        }
        else if ((messageRadio === 'text') && (ttsVoiceSpeed?.length === 0)) {
            setp4("active")
    ...
    const getFormData = () => {
        ...
        if (p1 && p2 && p3 && p4 && p5 && p6 && p7 && p8 === 'completed') {
            ...
            if ((messageRadio === "audio") && (whichAudio === 'Single')) {
            ...
            else if ((messageRadio === "audio") && (whichAudio === 'Multiple')) {
                formData = new FormData();
                let data = {
                    "campaign_name": templateName,
                    "startdate": `${dayjs(scheduleFrom).format('YYYY-MM-DD')} ${dayjs(startTime).format('HH:mm')}:00`,
    ...
    const onSubmitHandler = () => {
        ...
        getFormData()
            ...
            .then((resp) => {
                setredirectStatus(resp?.payload.status)
                response(resp?.data)
                if (resp?.payload.status === 200) {
                    ...
                    <div className="my-2 w-100" >
                        ...
                        <div className="d-flex justify-content-center">
                            <ToggleButtonGroup
                                ...
                                value={personalized}
                                exclusive
                                onChange={(e, v) => { setpersonalized(v) }}
                            ...
                            <div>
                                <div className="form-check m-2">
                                    <div className='d-flex align-items-center'>
                                        <input className="form-check-input mx-2 border-secondary" type="radio" name="messageRadio" onChange={(e) => { setmessageRadio(e.target.value )}} id="messageRadio" value="audio" disabled={(personalized === 'Yes') || (p3 === 'active' ? true : false)} />
                                        <label className="form-check-label mx-2" htmlFor="">
                                            Audio file
                                        </label>

                                        <div className='d-flex justify-content-around'>
                                            <CommanButton onClick={() => { setwhichAudio('Single') }} className='btnBack btn m-2' disabled={(messageRadio === 'text' ? true : false) || (p3 === 'active' ? true : false)}  >Single Prompt</CommanButton>
                                            <CommanButton onClick={() => { setwhichAudio('Multiple') }} className='btnBack btn m-2' disabled={(messageRadio === 'text' ? true : false) || (p3 === 'active' ? true : false)} >Multi Prompt</CommanButton>
                                        </div>
                                    </div>



                                    <div className='mx-4' style={messageRadio === 'audio' ? { display: 'block' } : { display: 'none' }} >
                                        <div>
                                            {
                                                whichAudio === "Single"
                                                    ?
                                                    <div>
                                                        ...
                                                        <div className='d-flex'>
                                                            <div className="mx-2">
                                                                <input type="file" accept='.wav' className="form-control border-secondary" id="audioMessage" onChange={(e) => { (e.target.files[0].size <= 2000000) ? setaudioMessage(e.target.files[0]) : toast.error("file must be less than 2MB") }} disabled={(messageRadio === 'text' ? true : false) || (p3 === 'active' ? true : false)} />
                                                            ...
                                                            <div className="mx-2">
                                                                <CommanButton type="submit" className="btnBack mb-3" onClick={messageTypeValidation} disabled={(messageRadio === 'text' ? true : false) || (p3 === 'active' ? true : false)} >Submit</CommanButton>
                                                            </div>
                                                            <div className="col-auto">

                                                                <CommanButton type="submit" className="btnBack mb-3" onClick={() => audioPlay(audioMessage, "audioPreview")}
                                                                    disabled={(messageRadio === 'text' ? true : false) || (p3 === 'active' ? true : false) || (audioMessage === null ? true : false)} >{audioPreview ? "Pause" : "Play"}</CommanButton>
                                                            </div>
                                                        </div>
                                                    </div>
                                                    :
                                                ...
                                                whichAudio === "Multiple"
                                                    ...
                                                    <div>
                                                        ...
                                                        <div className='d-flex justify-content-between'>
                                                            ...
                                                            <div className="mx-2">
                                                                <input type="file" accept='.wav' className="form-control border-secondary" id="welcomeFile" onChange={(e) => { (e.target.files[0].size <= 2000000) ? setwelcomeFile(e.target.files[0]) : toast.error("file must be less than 2MB") }} disabled={(messageRadio === 'text' ? true : false) || (p3 === 'active' ? true : false)} />
                                                                <label style={{ display: 'flex', justifyContent: 'flex-end', fontSize: '0.7rem', color: '#ff0202c7' }}> *Extension Must be .wav file(maxsize-2mb)</label>
                                                            </div>
                                                            <div className="col-auto">
                                                                <CommanButton type="submit" className="btnBack mb-3" onClick={() => audioPlay(welcomeFile, "welcomePreview")}
                                                                    disabled={(messageRadio === 'text' ? true : false) || (p3 === 'active' ? true : false) || (welcomeFile === null ? true : false)} >{welcomePreview ? "Pause" : "Play"}</CommanButton>
                                                            </div>
                                                        </div>
                                                        <div className='d-flex justify-content-between'>
                                                            <label className="form-check-label mt-1 " htmlFor="">Menu file</label>
                                                            <div className="mx-2">
                                                                <input type="file" accept='.wav' className="form-control border-secondary" id="menuFile" onChange={(e) => { (e.target.files[0].size <= 2000000) ? setmenuFile(e.target.files[0]) : toast.error("file must be less than 2MB") }} disabled={(messageRadio === 'text' ? true : false) || (p3 === 'active' ? true : false)} />
                                                                <label style={{ display: 'flex', justifyContent: 'flex-end', fontSize: '0.7rem', color: '#ff0202c7' }}> *Extension Must be .wav file(maxsize-2mb)</label>
                                                            </div>
                                                            <div className="col-auto">
                                                                <CommanButton type="submit" className="btnBack mb-3" onClick={() => audioPlay(menuFile, "menuPreview")}
                                                                    disabled={(messageRadio === 'text' ? true : false) || (p3 === 'active' ? true : false) || (menuFile === null ? true : false)} >{menuPreview ? "Pause" : "Play"}</CommanButton>
                                                            </div>
                                                        </div>
                                                        <div className='d-flex justify-content-between'>
                                                            <label className="form-check-label mt-1 " htmlFor="">Thanks file</label>
                                                            <div className="mx-2">
                                                                <input type="file" accept='.wav' className="form-control border-secondary" id="thanksFile" onChange={(e) => { (e.target.files[0].size <= 2000000) ? setthanksFile(e.target.files[0]) : toast.error("file must be less than 2MB") }} disabled={(messageRadio === 'text' ? true : false) || (p3 === 'active' ? true : false)} />
                                                                <label style={{ display: 'flex', justifyContent: 'flex-end', fontSize: '0.7rem', color: '#ff0202c7' }}> *Extension Must be .wav file(maxsize-2mb)</label>
                                                            </div>
                                                            <div className="col-auto">
                                                                <CommanButton type="submit" className="btnBack mb-3" onClick={() => this.audioPlay(thanksFile, "thanksPreview")}
                                                                    disabled={(messageRadio === 'text' ? true : false) || (p3 === 'active' ? true : false) || (thanksFile === null ? true : false)} >{thanksPreview ? "Pause" : "Play"}</CommanButton>
                                                            </div>
                                                        </div>
                                ...
                                <div className="form-check m-2 ">
                                    <div className='d-flex align-items-center mt-2'>
                                        ...
                                        <CommanButton type="submit" className="btnBack mx-3" onClick={() => { setmodal((prev)=> !prev) }} disabled={(messageRadio === 'audio' ? true : false) || (p3 === 'active' ? true : false)} >Add Template</CommanButton>
                                    ...
                                    <Modal show={modal} onHide={() => { setmodal((prev)=> !prev) }} aria-labelledby="contained-modal-title-vcenter">
                                        ...
                                        <Modal.Body className="grid-example">
                                            <div className='d-flex justify-content-center '>
                                                <CommanButton onClick={() => { whichMessage = 'write' }} className='btnBack btn m-2' >Writing Message</CommanButton>
                                                <CommanButton onClick={() => { whichMessage = 'exist' }} value='exist' className='btnBack btn m-2' >Existing Template</CommanButton>
                                            </div>
                                            <div>
                                                {
                                                    whichMessage === "write"
                                    ...
                                    <div className='m-2 d-flex'>
                                        <div className="mx-2">
                                            <textarea className="form-control border-secondary" placeholder="" id="ttsTextArea" rows={3} cols={25} disabled={(messageRadio === 'audio' ? true : false) || (p3 === 'active' ? true : false)} value={ttsTextArea} readOnly="true"  ></textarea>
                                        </div>


                                        <div className='mx-2'>
                                            <div className="input-group mb-3 ">
                                                <select className="form-select border-secondary" id="ttsVoiceTone" onChange={(e) => { setttsVoiceTone(e.target.value) }} disabled={(messageRadio === 'audio' ? true : false) || (p3 === 'active' ? true : false)}>
                                            ...
                                            <div className="input-group ">
                                                <select className="form-select border-secondary" id="ttsVoiceSpeed" onChange={(e) => { setttsVoiceSpeed(e.target.value) }} disabled={(messageRadio === 'audio' ? true : false) || (p3 === 'active' ? true : false)} >
                                            ...
                                            <div className='mx-2 my-2 '>
                                                <Tooltip title="Put var value for existing selected template" >
                                                    <input type="text" id='textData' className='form-control border-secondary w-50 m-auto' placeholder='Text Data'
                                                        disabled={(messageRadio === 'audio' ? true : false) || (p3 === 'active' ? true : false)} onChange={(e) => { settextData(e.target.value)}} />
                                                </Tooltip>
                                            </div>
                                            : undefined
                                    }
                                    <div className="mx-2 d-flex justify-content-center">
                                        <CommanButton type="submit" className="btnBack m-2" onClick={handleGenerateSpeech} disabled={(messageRadio === 'audio' ? true : false) || (p3 === 'active' ? true : false)}>Preview</CommanButton>
                                        <div style={{ display: 'none' }}>
                                            {Url && <audio controls src={Url} autoPlay="true" />}
                                        </div>

                                        <CommanButton type="submit" className="btnBack m-2" onClick={messageTypeValidation} disabled={(messageRadio === 'audio' ? true : false) || (p3 === 'active' ? true : false)}>Submit</CommanButton>
                                    </div>
